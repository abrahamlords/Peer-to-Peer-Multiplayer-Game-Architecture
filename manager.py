...
users = []    # users not playing a game  # {"user": user, "address": address, "port": port}
...
def startGame(dealer, k):
    ...
    if 0 < playerQuant < 4:  # if correct number of players
        ...
        for p in newGamePlayers:
            strang += "\n" + str(p)
        serverSocket.sendto(strang.encode(), (dealerInfo["address"], int(dealerInfo["port"])))
        for i in range(playerQuant):
            serverSocket.sendto(strang.encode(), (newGamePlayers[i]["address"], int(newGamePlayers[i]["port"])))
        blockSend = 1
        return strang
    else:
...
def queryGames():
    if len(games) == 0:
        ...
        for i in range(len(games)):
            strang += '\t Game ID: ' + str(games[i]['id']) +' Dealer: ' + games[i]['dealer'] + '\n'
            strang += '\t Players: \n'
