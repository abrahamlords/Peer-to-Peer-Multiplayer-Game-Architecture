...
myShit = [{'hand': [], 'score': 0}, {'hand': [], 'score': 0}]
discardPile = []
currentPlayerI = 0
...
def pickACard():
    ...
    if choice == 0:
        card = dek.getTopCard()
        print(currentPlayer['user'] + ' picked a card from the deck')
        sendMessage('You picked a card from the deck')
    ...
    if choice == 1:
        ...
        print(currentPlayer['user'] + ' picked a card from the discard pile')
        sendMessage('You picked a card from the discard pile')
    ...
    if choice == 2:
        ...
        print(currentPlayer['user'] + ' stole ' + stolenCard.toString() + ' for *** from ' +
              contacts[otherPlayerI + 2]['user'])
        sendMessage('You stole ' + stolenCard.toString() + ' for *** from ' +
              contacts[otherPlayerI + 2]['user'])
        message = currentPlayer['user'] + ' stole ' + stolenCard.toString() + ' from you for ' + card.toString()
        time.sleep(1.2)
        clientSocket.sendto(message.encode(), (contacts[otherPlayerI + 2]["address"], int(contacts[otherPlayerI + 2]["port"])))
    ...
    for p in contacts[2:]:
        ...
        for i in range(3):
            if topRow[i].value == bottomRow[i].value:
                ...
                myShit[ii]['score'] += scoreChart(topRow[i])
                myShit[ii]['score'] += scoreChart(bottomRow[i])
        print(currentPlayer['user'] + '\'s score is ' + str(myShit[ii]['score']))
        sendToAllParts(currentPlayer['user'] + '\'s score is ' + str(myShit[ii]['score']))
        ii = (ii + 1) % 2
...
def announceWinner():
    if myShit[0]['score'] < myShit[1]['score']:
        sendToAllParts(contacts[3]['user'] + ' wins!')
    else:
        sendToAllParts(contacts[2]['user'] + ' wins!')


def endGame():
    announceWinner()
    sendToAllParts('Goodbye!')

    message = 'end ' + str(myGameID) + ' ' + myContactInfo[0]
    time.sleep(1.2)
    clientSocket.sendto(message.encode(), (contacts[0]["address"], int(contacts[0]["port"])))
    pass
...
if __name__ == '__main__':
    ...
    while True:
        ...
        command = input("Send a command to the main server:\n>")
        if (command == 'join'):
            joinGame()
            playGame()
        elif (command == 'start'):
            ...
            time.sleep(1.2)
            clientSocket.sendto(command.encode(), (contacts[receiverKey]["address"], int(contacts[receiverKey]["port"])))
            response, serverAddress = clientSocket.recvfrom(2048)  # buffer size is 2048
            print(response.decode())
