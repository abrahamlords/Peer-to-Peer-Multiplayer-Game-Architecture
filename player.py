...
# manager: register, query players,query games,and de-register
"""
...
myShit = [{'hand': []}, {'hand': []}, {'hand': []}] # convert these to sends
...
def dealCards():
    ...
    for i in range(6):
        ...
        for p in contacts[2:]:
            ...
            #myShit[ii]['hand'].append(card) # TODO receive card
            ii = (ii + 1) % 3
...
def PLAYBALL():
    ...
    for round in range(1, 2): # TODO 1,10
...
discardPile = []
currentPlayerI = 0
currentPlayer = 'contacts[2]' #TODO changed this to string
...
def playAsDealer():
    ...
    for round in range(1, 2):  # TODO 1,10
        ...
        dealCards()
        # turn2cardsUp() done by participant
        '''displayAllHands('start')
        allCardsUp = False
        while not allCardsUp:
            allCardsUp = True
...
def sendMessage(receiverKey, message):
    print(response)

def sendCommand(receiverKey, command, args):
    message = command + ' ' + ' '.join(args)
    clientSocket.sendto(message.encode(), (contacts[receiverKey]["address"], contacts[receiverKey]["port"]))
